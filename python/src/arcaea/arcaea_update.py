...
class arcaeaDataDict(TypedDict):
    last_update: number
    songs: list[arcaeaData]
...
def arcaea_updade(data_path: str, jackets_path: str):
    ...
    """
    data: arcaeaDataDict = json_funks.get(data_path)
    ...
    def update(update_url: str):
        ...
        try:
            ...
            for row in table_rows:
                try:
                    th: str = row.find('th').text
                    td: list = list(map(lambda x: x.text, row.find_all('td')))
                    rows[th] = td
                except Exception:
                    pass
            # 楽曲データが既にあるなら一旦削除
            ...
            if id == 0:
                id = max_id + 1
            if title in ["Quon", "PRAGMATISM(PRAGMATISM -RESURRECTION-)"]:
                return
            # 楽曲データを追加
            data["songs"].append({
                'title': title,
