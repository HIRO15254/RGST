...
def analyse_arcaea_result(settings_path: str, data_path: str, jackets_path: str, nums_path: str, result_path: str, *image_paths: tuple[str]):
    ...
    def checkJacket(trimmed_image: numpy.ndarray) -> string:
        ...
        img_hists: list[numpy.ndarray] = []
        # BGRそれぞれについて、与えられた画像のヒストグラムを作成
        for i in range(3):
            img_hists.append(cv2.calcHist([resized_image], [i], None, [256], [0, 256]))
        best_num: int = 0
        best_val: float = 0
        for jacket_path, jacket_hists in jackets_hists.items():
            # BGRそれぞれについてヒストグラムの類似度を計算し、最も高い結果を保持
            ...
            if best_val < ret:
                best_num = int(re.search(r"jacket(\d{1,3})[abcdefg]\.npz", jacket_path).group(1))
                best_val = ret
