...
TESSDATA_PATH = 'C:/Users/PC_User/AppData/Local/Programs/Tesseract-OCR/tessdata'
...
def getJudgePlace(img, sco_x, sco_y, sco_w, sco_h):
    lower = (0, 0, 90)
    upper = (180, 50, 130)
    img_hsv = cv2.cvtColor(img[sco_y + sco_h:, sco_x:sco_x + sco_w], cv2.COLOR_BGR2HSV)
    ...
    for j in range(0, len(contours)):
        if len(contours[j]) > 0:
            ...
            # 小さい範囲は無視
            if cv2.contourArea(contours[j]) < 4:
                continue

            rect = contours[j]
            x, y, w, h = cv2.boundingRect(rect)
            # 上端がジャケットの上端より下にあり、横幅が縦幅の5倍以上を満たす範囲のうち最大の範囲を返す
            cv2.rectangle(bin_img, (x, y), (x + w, y + h), 128, 1)
    cv2.imshow("a", bin_img)
    cv2.waitKey(0)
...
for i in range(0, 34):
    ...
    imgs = []
    for j in range(3):
        imgs.append(cv2.imread(f'image/ss{i * 3 + j + 1}.png'))
